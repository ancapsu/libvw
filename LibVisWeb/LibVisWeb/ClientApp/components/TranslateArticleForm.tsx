...
type InjectedTranslateArticleFormProps =
    TranslateArticleBaseProps &
    TranslateArticleFormProps &
    InjectedFormProps<ArticleModel.NewArticleModel>;

class TranslateArticleForm extends React.Component<InjectedTranslateArticleFormProps, {}> {
    ...
    constructor(props: InjectedTranslateArticleFormProps, context: any) {
    ...
    componentDidMount() {
        ...
        else
        {
            ...
            this.props.requestArticle(this.props.articleid, this.props.lang);
                        
        }

    }

    componentWillReceiveProps(props: InjectedTranslateArticleFormProps) {
        ...
        if (!this.props.hasEditBase && props.hasEditBase && props.article != null) {

            this.contentReady = true;
            this.forceUpdate();
            
            this.props.initialize({ Title: props.article.Title, Text: props.article.Text })
            ...
            for (var j = 0; j < props.categories.length; j++) {
                ...
                for (var k = 0; k < props.article.Categories.Categories.length; k++) {

                    if (props.categories[j].Label == props.article.Categories.Categories[k].Label)
                        tem = true;
                            ...
                            <div className="td-pb-border-top">
                                <div className="td-block-title-wrap">
                                    <h4 className="td-block-title">
                                        <span className="news-page-headline">{RacMsg.Get(RacMsg.Id.EditTheTranslation)}</span>
                                    </h4>
                                </div>
                            </div>
...
const DecoratedTranslateArticleForm = reduxForm<ArticleModel.NewArticleModel>({ form: "translateArticleForm" })(TranslateArticleForm as any) as any;
...
)(DecoratedTranslateArticleForm) as any;
